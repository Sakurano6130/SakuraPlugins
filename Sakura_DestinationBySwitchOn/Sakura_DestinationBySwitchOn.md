...
| ver   | 日付       | 説明                                                                                                                                                       |
| ----- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.2.0 | 2025/09/21 | アーカイブシーンを開く際に、特定のアーカイブを指定して直接開く機能を追加                                                                                   |
| 2.1.7 | 2025/02/27 | アーカイブシーンでマウスの右クリックのキャンセルに対応                                                                                                     |
| 2.1.6 | 2024/12/17 | ブラウザ版で ReferenceError: require is not defined のエラーが出ないように修正                                                                             |
| 2.1.5 | 2024/11/25 | アーカイブシーンでピクチャを指定したときだけに小目的が表示されていたため、表示されないように修正                                                           |
| 2.1.4 | 2024/11/23 | サブ目的のスイッチオンオフ時にサブ目的表示がリフレッシュされるように修正                                                                                   |
| 2.1.3 | 2024/11/06 | アーカイブシーンのコマンドウィンドウの位置調整機能を追加                                                                                                   |
| 2.1.2 | 2024/11/06 | アーカイブシーンのコマンドウィンドウ背景を黒表示から通常のウィンドウに変更<br>アーカイブシーンのコマンドウィンドウが達成済みでも選択できてしまったため修正 |
| 2.1.1 | 2024/11/06 | アーカイブシーンのコマンドウィンドウ背景が透過しないように変更<br>アーカイブシーンのコマンドウィンドウの「やめる」がキャンセルできていなかったため修正     |
| 2.1.0 | 2024/10/13 | ストーリーが進んだ時に、音を鳴らせるように                                                                                                                 |
...
#### 特定のアーカイブを直接開くオプション追加 `ver.2.2`


  ![alt text](image-64.png)

- 特定のアーカイブを直接開きたい場合は、「直接開くｱｰｶｲﾌﾞのﾀｲﾄﾙ」を指定してください。
- 複数ヒットする場合は、1件目に見つかったものが開かれます。
- 見つからなかった時の挙動を、以下から選べます。

    ![alt text](image-62.png)

- 注）目的にスイッチが紐づけられている場合は、そのスイッチがオンにならないと「見つからない」扱いになります。
...
- **storyProgressAudio** `ver.2.1`
  - 目的が変わった時に鳴らす音です。指定しなかった場合は鳴りません。
...
- **commandWindowOffsetX** `ver.2.1.3`
  - 達成済ｳｨﾝﾄﾞｳのX軸位置調整です

- **commandWindowOffsetY** `ver.2.1.3`
  - 達成済ｳｨﾝﾄﾞｳのY軸位置調整です

![alt text](image-59.png)

コマンドウィンドウの位置を調整することができます。
