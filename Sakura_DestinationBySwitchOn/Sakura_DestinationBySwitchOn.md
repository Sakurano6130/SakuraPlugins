...
### ①プラグインコマンドから呼び出す
  ...
  - このとき、シーンの左上に大きく出るタイトルと、表示する対象のカテゴリ（複数指定可）を設定できます。
...
### ②プラグインパラメータでメニューコマンドに登録する
  ...
  - 指定したカテゴリごとに独立した別のコマンドとして複数設定することもできます。

    ![alt text](image-29.png)

  - コマンド名、対象のカテゴリを指定してください。
  - カテゴリを指定しなかった場合は、コモンイベントに登録した全ての目的が対象となります。
  - 注）目的にスイッチが紐づけられている場合は、そのスイッチがオンにならないと表示されません。
  ...
  - 有効/無効を制御するスイッチID：このスイッチがオンになっていない場合は、メニューから呼び出すことができません。ストーリーが進むと解放されるメニューコマンドのような演出にご利用ください。
